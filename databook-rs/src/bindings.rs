...
pub mod plugin {
  ...
  impl<T> Plugin<T> {
    ...
    get_state: impl Fn(&mut T) -> &mut PluginData + Send + Sync + Copy + 'static,
    ) -> anyhow::Result<()> {
      Ok(())
    }
    ...
    get_state: impl Fn(&mut T) -> &mut PluginData + Send + Sync + Copy + 'static,
      ...
      let invoke= instance.get_typed_func::<(i32,i32,), (i32,), _>(&mut store, "invoke")?;
      let memory= instance
    ...
    pub fn invoke(&self, mut caller: impl wasmtime::AsContextMut<Data = T>,input: & str,)-> Result<String, wasmtime::Trap> {
      ...
      let (result1_0,) = self.invoke.call(&mut caller, (ptr0, vec0.len() as i32, ))?;
